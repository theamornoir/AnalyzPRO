...
## AI-слой (единый Gemini-клиент, чистый)
- `internal/ai/gemini/client.go`: `Client`, `NewClient(cfg Config) (*Client, error)`. Конфигурация (ключ/модель/прокси/база/лимит) — ТОЛЬКО из `config.Config` (env через `config.Load`): `GOOGLE_GEMINI_API_KEY`, `GOOGLE_AI_MODEL` (дефолт `gemini.DefaultModel`=`gemini-2.5-flash`), `GEMINI_PROXY` (socks5/http; иначе системный `HTTPS_PROXY`), `GEMINI_API_BASE` (дефолт `gemini.DefaultAPIBase`), `GEMINI_MAX_CONCURRENCY` (дефолт `gemini.DefaultMaxConcurrency`=4; низкий предел защищает от 429 при низком RPM тарифа Gemini). Клиент НЕ читает env сам и НЕ логирует внутри (чистые функции); ошибка прокси → `NewClient` возвращает err, `app.go` логирует `LogGeminiProxyError` и фоллбэчит без прокси; пустой ключ → `NewClient` ОК, но вызовы возвращают `ErrGeminiKeyNotSet` (бот стартует).
- **ПРОМПТЫ И ВСЕ ТЕКСТЫ — В LOCALES:** системные промпты (`SystemPromptMedicalAnalyst/Analysis/AnalysisMulti/JSON/Dossier/BodyIntelligence`) и пользовательские (`PromptForAnalysisSummary/File/Files/AnalysisJSON/Bioscan/BodyScanJSON/DossierJSON`) в `internal/locales/prompts_gemini.go` (+ уже существующие `prompts_*.go`). НЕИСПОЛЬЗУЕМЫЕ заглушки `PromptForRegular`/`PromptForAthlete` (prompts_regular.go/prompts_athlete.go) УДАЛЕНЫ как мёртвый код эпохи Claude. Ошибки (`ErrGemini*`) в `internal/locales/errors.go`, логи (`LogGemini*`) в `internal/locales/logs.go`. Хардкода текстов/промптов в client.go НЕТ.
- Метод `GenerateWithFiles(ctx, systemPrompt, prompt, files []Attachment, maxTokens)` — промпт + ВСЕ файлы в ОДНО сообщение (inlineData: image/* + application/pdf). Таймаут `DefaultTimeout`=120с.
- **AI-СЕМАФОР:** поле `Client.sem = semaphore.NewWeighted(cfg.MaxConcurrency)` (golang.org/x/sync), а не глобальная переменная. В `generateWithConfig`: `sem.Acquire(ctx,1)` + `defer sem.Release(1)`. Защита от 429 и OOM.

- **НАДЁЖНОСТЬ (retry/safety):** `doRequestWithRetry` повторяет запрос `defaultMaxRetries=3` раза с экспоненциальным backoff (старт `defaultRetryBackoff=400ms`, x2) на сетевую ошибку и на HTTP 429/5xx. 2xx и 4xx (кроме 429) НЕ повторяются (400 = bad request, повтор бессмысленен). Backoff уважает `ctx.Done()` (`sleepWithCtx`). В каждый запрос добавляется `safetySettings` со всеми категориями в `BLOCK_NONE` - иначе мед. контент (анализы/фото тела) Gemini ложно блокирует → пустой ответ. Тюнинг повторов/температуры - константы в client.go (не env: ключи/модели/прокси уже в config).
- **ТЕСТЫ** `internal/ai/gemini/client_test.go`: `TestRetryOn5xx` (2x500→200), `TestRetryExhausted` (все 503→`ErrGeminiRetryExhausted`), `TestNoRetryOn400` (400 без повтора), `TestJSONRequestConfig` (проверяет `responseMimeType`/`temperature`/`safetySettings` в теле), `TestKeyNotSet` (`ErrGeminiKeyNotSet`, без сетевого вызова). `go test ./internal/ai/gemini/ -v`.
...
- Любой текстовый результат ИИ, уходящий ПРЯМО В ЧАТ (не файлом PDF/HTML), отправлять только через `helpers.SendLongMessagePlain(ctx,b,chatID,text)` (режет по 4000 рун). Прямой `b.SendMessage(SendMessageParams{Text: result})` без дробления — БАГ: при >4096 Telegram вернёт 400, сообщение не дойдёт, хотя бот пошлёт «готово» и сохранит результат в «Мой профиль». Чистая логика — `helpers.SplitLongMessage`.
- Зашито: обычный/мульти-файловый анализ (`upload_process.go` `sendLongMessagePlain`), текстовый анализ (`upload_text.go`), базовый Bioscan (`bioscan_basic.go`), консультация (`router_menu_actions.go` `sendLongMessage`). Расширенный анализ / Bioscan PRO / досье уходят файлом `SendDocument` — чанковать не нужно.

## Готчи при правке (окружение)
- ⚠️ Инструмент записи файлов (write_file / replace_text) в этом окружении может НЕ сохранять изменения на диск, хотя сообщает «успешно»: наблюдалось пустой файл (`*_test.go` → `expected 'package', found 'EOF'`) и дубль первой строки `package` (обе строки `package X`). Затронутые: `webapp_files/index.html`, `.opencodex/memory.md`, `upload_sendlong_test.go`, `helpers/send_long_test.go`. ПОСЛЕ каждой правки перепроверяй файл на диске (`wc -l` / `grep`) и при необходимости перезаписывай через `cat > file <<'EOF'` в shell (надёжно). Чтение через read_file всё равно показывает актуальный (правильный) контент.
