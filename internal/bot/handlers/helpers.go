...
func downloadUploadedFile(
	...
	realMime := detectRealMimeType(data, fileName)
...
func processAllPhotos(
	...
	if len(allFileData) == 1 {
		...
		if len(allResults) == 0 {
			safeDeleteLoadingMsgs(ctx, b, chatID, loadingMsg, textMsg)
			sendError(ctx, b, chatID)
			stateManager.Reset(chatID)
			return
		}
...
func processPDF(
	...
	pdfMime := detectRealMimeType(fileData, "file.pdf")
	if pdfMime == "application/octet-stream" {
		pdfMime = "application/pdf"
	}

	result, err := analysisService.HandleAnalysisFromFileWithContext(ctx, fileData, pdfMime, analysisText)
	if err != nil {
		safeDeleteLoadingMsgs(ctx, b, chatID, loadingMsg, textMsg)
		sendError(ctx, b, chatID)
		return
