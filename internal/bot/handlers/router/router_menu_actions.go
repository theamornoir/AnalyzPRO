...
func (r *router) handleExtendedAnalysis(ctx context.Context, b *tgbot.Bot, chatID int64) bool {
	...
	r.deleteHubBlock(ctx, b, chatID)
	// Чистый старт опросника: сбрасываем смещение прогресса (никакие вопросы
	// ещё не пропущены, счётчик «Вопрос N из 7» будет абсолютным).
	r.stateManager.SetUserData(chatID, userdata.HealthSkipKey, "0")
	// Если у пользователя уже есть сохранённый профиль (включая взятый из
	// Mini App «Мой профиль», т.к. tryProfileConfirm сверяет ОБА хранилища) -
	// показываем экран «Данные уже известны? Использовать / Изменить» вместо
	// молчаливой подстановки. «Использовать» пропускает демографию и ведёт к
	// вопросам об образе жизни; «Изменить» запускает опросник заново.
