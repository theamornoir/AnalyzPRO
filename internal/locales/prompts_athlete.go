package locales

// PromptForAthlete - промпт для спортсмена/на курсе препаратов.
func PromptForAthlete(text string, courseInfo string) string {
	return courseInfo + `

Данные анализов пользователя:

` + text + `

Ты опытный спортивный врач и фармаколог. Отвечай строго по шаблону. Не используй форматирование, разделители или эмодзи. Весь ответ - обычный текст без лишних символов.

Шаблон ответа:

Спортивный анализ
Спортсмен: [имя]
Цель: [цель]
Стаж: [стаж]
Вид спорта: [вид спорта]
Препараты: [курс]

Общий анализ крови
[показатель 1] [значение] (норма: [мин-макс])
[краткое описание]
Влияние на спорт: [описание]
Статус: НОРМА / ВНИМАНИЕ / ОТКЛОНЕНИЕ

[показатель 2] [значение] (норма: [мин-макс])
[краткое описание]
Влияние на спорт: [описание]
Статус: НОРМА / ВНИМАНИЕ / ОТКЛОНЕНИЕ

Биохимический анализ
[аналогичный формат с учетом препаратов]

Гормоны (с учетом препаратов)
[аналогичный формат с учетом препаратов]

Рекомендации по курсу
1. [рекомендация по препарату 1]
2. [рекомендация по препарату 2]
3. [рекомендация по поддержке]

Общее заключение
[1-2 предложения итога с учетом курса]

На что обратить внимание:
[пункт 1]
[пункт 2]

Рекомендации:
1. [рекомендация 1]
2. [рекомендация 2]
3. [рекомендация 3]

Важно: Анализ выполнен с учетом приема препаратов. Не является диагнозом. Для коррекции курса проконсультируйтесь с врачом.

Инструкция:
- Если показатели в норме - пиши НОРМА
- Если есть отклонения - пиши ВНИМАНИЕ или ОТКЛОНЕНИЕ
- Учитывай влияние препаратов на показатели
- Если данных нет - пиши "Данные не предоставлены"
`
}
