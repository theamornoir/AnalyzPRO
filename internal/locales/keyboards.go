...
const (
	...
	// «Сводка здоровья» прямо из чата (появляется после выдачи отчёта, чтобы
	// пользователь мог сразу открыть сохранённый результат).
	BtnOpenHealthSummary = "📊 Открыть Сводку здоровья"
)
...
const (
	...
	// результат сохранён в «Сводку здоровья», оформлен там аккуратно и его
	// можно открыть прямо сейчас (кнопка ниже) либо позже через меню
	// Здоровье → Сводка здоровья. Короткое, без форматирования.
	MsgResultSavedSummary = "✅ Результат сохранён в «Сводке здоровья». Там он оформлен аккуратно и его легко найти позже. Открыть можно прямо сейчас по кнопке ниже, либо в меню: Здоровье → Сводка здоровья."
	...
	// попадёт результат и даёт кнопку открыть демо-Сводку (чтобы увидеть,
	// как там оформлены сохранённые отчёты).
	MsgDemoResultSavedSummary = "🧪 Это демо-режим: результаты здесь не сохраняются. Но в обычном режиме каждый анализ и Bioscan попадают в «Сводку здоровья» - оформленные аккуратно и доступные позже через меню Здоровье → Сводка здоровья. Можно посмотреть, как это выглядит:"
)
