...
const (
	...
	// 🧪 Демо-кнопки для ПОВТОРНЫХ (сравнительных) отчётов: имитируют
	// второй проход расширенного анализа/биоскана и показывают отчёт с
	// блоком сравнения с «предыдущим» (что улучшилось / что улучшить).
	BtnExtendedAnalysisRepeatDemo = "🧪 Демо: повтор. анализ"
	BtnBioscanExtendedRepeatDemo  = "🧪 Демо: повтор. Bioscan"
...
const (
	...
	// MsgReportProgressNote - дописывается к сообщению выдачи расширенного
	// анализа и Bioscan PRO. Сообщает пользователю, что повторные отчёты
	// сравниваются между собой и показывают прогресс в «Сводке здоровья».
	MsgReportProgressNote = "🔁 Повторные отчёты автоматически сравниваются с предыдущими - динамику (что улучшилось, что подтянуть) видно в «Сводке здоровья». Откройте 📊 Здоровье → 💡 Сводка здоровья."
	// MsgDemoRepeatBanner - пояснение к демо повторного (сравнительного)
	// отчёта: это имитация ВТОРОГО прохода, показывающая блок сравнения.
	MsgDemoRepeatBanner = "🧪 ДЕМО-режим - ПОВТОРНЫЙ отчёт (сравнение с предыдущим). Так выглядит результат второго расширенного анализа/биоскана: отчёт строится не «с нуля», а с блоком сравнения (что улучшилось, что улучшить). В реальном режиме сравнение происходит автоматически при повторном прохождении и отображается в «Сводке здоровья»."
)
